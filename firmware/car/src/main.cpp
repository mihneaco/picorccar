...
void print_ble_packet(const RemoteLink::Packet& p_packet)
{
    if (p_packet.m_truncated)
    {
        LOG_TRACE("BLE packet len=%u copied=%u overwritten=%lu",
                  static_cast<unsigned>(p_packet.m_total_length),
                  static_cast<unsigned>(p_packet.m_copied_length),
                  static_cast<unsigned long>(p_packet.m_overwritten_packets));
    }
    else
    {
        LOG_TRACE("BLE packet len=%u overwritten=%lu",
                  static_cast<unsigned>(p_packet.m_total_length),
                  static_cast<unsigned long>(p_packet.m_overwritten_packets));
    }
}
    ...
    */
    ...
    while (true)
    {
        ...
        */
        ...
        if (res)
        {
            last_packet_ms = latest_packet.m_received_ms;
            print_ble_packet(latest_packet);
        }
