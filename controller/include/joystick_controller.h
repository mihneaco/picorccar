#pragma once

#include <cstdint>
#include <optional>

#include "pinout.h"

namespace pinout
{
    struct JoystickControllerPins
    {
        Pin m_bpressed{};
        Pin m_x_axis{};
        Pin m_y_axis{};
    };

    constexpr JoystickControllerPins JOYSTICK_PINS{
        pinout::JOYSTICK_BPRESSED_GPIO,
        pinout::JOYSTICK_X_AXIS_GPIO,
        pinout::JOYSTICK_Y_AXIS_GPIO};
}

class JoystickController
{
public:
    struct Sample
    {
        /**
         * @brief Value domain of the axis readings: raw ADC counts in [0, ADC_MAX_VALUE],
         *        resting center assumed at mid-scale.
         */
        static constexpr std::uint8_t ADC_INPUT_BITS = 12;
        static constexpr std::uint16_t ADC_MAX_VALUE = (1u << ADC_INPUT_BITS) - 1u;
        static constexpr std::uint16_t ADC_CENTER = (ADC_MAX_VALUE + 1) / 2;

        /// Deleted so a Sample can only exist via the clamping constructor.
        Sample() = delete;
        /** @brief Clamp both axes to [0, ADC_MAX_VALUE]. */
        Sample(bool p_bpressed, std::uint16_t p_x_axis, std::uint16_t p_y_axis);

        bool m_bpressed{};
        std::uint16_t m_x_axis{};
        std::uint16_t m_y_axis{};

        /// @brief Largest deflection from the resting center across both axes, in ADC counts.
        std::uint16_t max_center_offset() const;
    };

    /** @brief Bind GPIO/ADC pins. Does not touch hardware; call init() for that. */
    explicit JoystickController(pinout::JoystickControllerPins p_pins);

    /** @brief Validate pins and init the button GPIO and ADC inputs. */
    bool init();
    /** @brief Read the button and both axes; nullopt if not initialized. */
    std::optional<Sample> read() const;

private:
    pinout::JoystickControllerPins m_pins;
    bool m_initialized = false;
};
