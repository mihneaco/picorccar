...
/** @brief Whether p_pin is one of the ADC-capable GPIOs. */
...
constexpr AdcPinMapEntry ADC_GPIO_MAP[] = {{26, 0}, {27, 1}, {28, 2}, {29, 3}};
/** @brief Map a GPIO to its ADC input number; 0 if not found. */
...
/** @brief Init a GPIO's direction and pull mode. */
...
inline bool read_gpio_input(const Pin p_pin) { return gpio_get(p_pin); }

/** @brief Drive a GPIO output level. */
...
inline void init_adc() { adc_init(); }

/** @brief Enable a GPIO for ADC input. */
inline void init_adc_gpio_pin(const Pin p_pin) { adc_gpio_init(p_pin); }

/** @brief Blocking read of one ADC input. */
...
/** @brief Configure a GPIO for PWM at the target frequency/wrap and set its initial level. */
...
/** @brief Set a PWM GPIO's output level/duty. */
 ...
 */
inline void enable_led() { cyw43_arch_gpio_put(CYW43_WL_GPIO_LED_PIN, true); }

/** @brief Turn the on-board status LED off. */
inline void disable_led() { cyw43_arch_gpio_put(CYW43_WL_GPIO_LED_PIN, false); }
