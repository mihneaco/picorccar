...
/** @brief Init uLog and subscribe the pico stdio sink at the given threshold. */
...
inline void
log(const ulog_level_t severity, const char* const function, const char* const format, ...)
{
#ifdef ULOG_ENABLED
    ...
    std::vsnprintf(&message[prefix_length],
                   sizeof(message) - static_cast<std::size_t>(prefix_length),
                   format,
                   args);
    va_end(args);
